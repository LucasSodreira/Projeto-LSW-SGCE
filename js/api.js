...
const RESOURCE_ALIASES = {
  ...
  competitions: ['/competicoes','/competitions','/competicao','/torneios','/torneio']
};
    ...
    try {
      ...
      if(!res.ok){
        ...
        if(res.status !== 404 || DEBUG_DISCOVERY){
          if(DEBUG_DISCOVERY) console.warn('[API discovery]', baseKey, 'fail', c, res.status);
        }
      ...
      if(Array.isArray(data)){
        if(DEBUG_DISCOVERY) console.info('[API discovery]', baseKey, 'resolved to', c);
...
async function tryAliases(baseKey, method, body, params){
  ...
  const doFetch = async (p)=>{
    ...
    const options = method==='GET'? undefined : { method, headers:{'Content-Type':'application/json'}, body: body? JSON.stringify(body): undefined };
  ...
  if(hasAxios && method==='GET'){
    ...
    if(method==='POST'){ const {data}= await axios.post(target, body); return data; }
    if(method==='PUT'){ const {data}= await axios.put(target, body); return data; }
    if(method==='PATCH'){ const {data}= await axios.patch(target, body); return data; }
    if(method==='DELETE'){ await axios.delete(target); return true; }
  }
  return doFetch(path);
}
...
const CompetitionsAPI = {
  ...
  update: async (id,data) => { const base = await resolveResource('competitions'); return apiPut(`${base}/${id}`, data); },
  patch: async (id,data) => { const base = await resolveResource('competitions'); return apiPatch(`${base}/${id}`, data); },
  remove: async (id) => { const base = await resolveResource('competitions'); return apiDelete(`${base}/${id}`); }
};
const TeamsAPI = {
  list: () => apiGet('/teams'),
  get: async (id) => {
